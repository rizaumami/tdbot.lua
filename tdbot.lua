...
-- @limit Maximum number of messages to be returned, can't be greater than 100. There may be less than limit messages returned even the end of the history is not reached
...
function tdbot.searchChatMessages(chatid, query, senderuserid, frommessageid, lim, searchmessagesfilter, callback, data)
  assert (tdbot_function ({
...
-- @protocol Description of supported by the client call protocols
function tdbot.acceptCall(callid, protocol, callback, data)
  assert (tdbot_function ({
    _ = 'acceptCall',
    call_id = callid,
...
function tdbot.addStickerToSet(userid, name, sticker, callback, data)
  assert (tdbot_function ({
    _ = 'addStickerToSet',
    user_id = userid,
    name = tostring(name),
